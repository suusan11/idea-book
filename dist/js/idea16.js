...
var els = [];
var elNum = 19; // function createElm() {
...
function setRandomColor() {
  for (var _i = 0; _i < elNum; _i++) {
    var colorNum = Math.floor(Math.random() * colorCode.length);
    var rondomColor = colorCode[colorNum];
    els[_i].style.backgroundColor = rondomColor; // gsap.set(els[i], {backgroundColor: rondomColor})
    // return rondomColor;
  }
}
...
function play() {
  ...
  tl.to(even, {
    ...
    y: function setMoveY() {
      for (var _i16 = 0; _i16 < elNum; _i16++) {
        var targetY = Math.floor(Math.random() * innerHeight - innerHeight / 2);
        return targetY;
      }
    },
